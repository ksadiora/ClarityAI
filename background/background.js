...
You MUST choose one: either EDUCATIONAL/INFORMATIONAL or MANIPULATIVE. Use the full scale. Do NOT default to neutral. When in doubt between neutral and manipulative, choose MANIPULATIVE with intensity 55+.
...
- Titles/descriptions that withhold information to force a click ("what happens next", "one weird trick", "doctors hate", "number X will shock you", "watch until the end", "must see", "going viral", "everyone's talking", "finally revealed", "the truth about", "insane", "unbelievable" used as hype) → "curiosity_gap", intensity 60-85
- Content designed to provoke strong emotion, outrage, or fear for engagement → "outrage" or "fear" or "controversy", intensity 55-90
...
NEUTRAL (only when truly neither—use sparingly):
- Bland entertainment, vague posts, normal vlogs with no pressure or sensationalism. Use neutral ONLY when content has no clear educational value AND no manipulation signals. If the title or description uses urgency, hype, or engagement bait, do NOT use neutral.
...
function parseGeminiClassification(text) {
  ...
  try {
    ...
    if (parsed.manipulative === true && (mechanic === 'neutral' || !parsed.mechanic)) mechanic = 'curiosity_gap';
    const intensity = Math.min(100, Math.max(0, parseInt(parsed.manipulation_intensity, 10) || 0));
    const finalIntensity = parsed.educational === true ? Math.min(intensity, 25) : (parsed.manipulative === true && intensity < 55 ? Math.max(55, intensity) : intensity);
    return {
      manipulation_mechanic: mechanic,
      manipulation_intensity: finalIntensity,
      urgency_signals: false,
      emotional_valence: 'neutral',
      content_category: 'other',
    };
...
const MANIPULATION_SIGNALS = (function () {
  ...
  const clickbait = /\b(you won't believe|what happens next|one weird trick|doctors hate|number \d+ will shock|watch until the end|must see|going viral|everyone's talking|finally revealed|the truth about|they don't want you to know|insane|unbelievable|mind.?blowing|shocking|secret that|exposed|breaking.*\b|viral|hype|hacked|leaked|banned|forbidden|they're hiding)\b/i;
  const outrageBait = /\b(outrage|outraged|disgusting|appalling|how dare|can't believe (this|they)|unacceptable|worst ever|ruined|destroyed|attack on|war on|against (us|you)|evil|corrupt)\b/i;
  return function (text) {
    const t = (text || '').toLowerCase();
    return scam.test(t) || aiGenerated.test(t) || conspiracy.test(t) || clickbait.test(t) || outrageBait.test(t);
  };
})();
...
function ensureManipulativeWhenSignals(content, classification) {
  ...
  const isOutrageBait = /\b(outrage|outraged|disgusting|appalling|how dare|can't believe (this|they)|unacceptable|worst ever|evil|corrupt)\b/i.test(text);
  ...
  else if (isOutrageBait) mechanic = 'outrage';
  return {
    ...classification,
    manipulation_mechanic: mechanic,
